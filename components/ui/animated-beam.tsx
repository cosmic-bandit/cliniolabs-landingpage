  ...
  useEffect(() => {
    const updatePath = () => {
      if (containerRef.current && fromRef.current && toRef.current) {
        ...
        // Use Cubic Bezier for smooth, organic curves like ss-10
        const dx = endX - startX
        const dy = endY - startY

        // Control points for smooth S-curve
        // First control point - horizontal departure from source
        const cp1x = startX + dx * 0.5
        const cp1y = startY - curvature

        // Second control point - horizontal arrival to target
